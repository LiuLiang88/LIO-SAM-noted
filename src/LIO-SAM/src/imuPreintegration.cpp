...
public:
    ...
    void odometryHandler(const nav_msgs::Odometry::ConstPtr &odomMsg)
    {
        ...
        // 确保我们已经进行过imu数据积分了 因为这个队列是进行储存数据积分的队列(第一帧imu数据会不积分直接push进队列中)
        if (imuQueOpt.empty())
            return;
        ...
        if (systemInitialized == false)
        {
            ...
            // pop old IMU message
            // 丢弃老的imu信息 将这个里程计消息之前的imu信息全部扔掉
            ...
            // 设置其初始位姿和置信度
            gtsam::PriorFactor<gtsam::Pose3> priorPose(X(0), prevPose_, priorPoseNoise);   //priorPoseNoise类似于置信度 也就是数据越可信这个值就越小
            // 约束加入到因子图中
            ...
            // initial bias
            // 初始化先验偏置信息为0并添加到因子图中
            prevBias_ = gtsam::imuBias::ConstantBias();//初始化
            gtsam::PriorFactor<gtsam::imuBias::ConstantBias> priorBias(B(0), prevBias_, priorBiasNoise);
            graphFactors.add(priorBias);
            // add values
            // 设置变量的初始估计值(将各个状态量赋成初始值,之前上边的add加的都是约束,现在添加的是状态)
            graphValues.insert(X(0), prevPose_);
            graphValues.insert(V(0), prevVel_);
            graphValues.insert(B(0), prevBias_);
            // optimize once
            // 将因子图(约束和状态量)更新到isam2优化器中
    ...
    void imuHandler(const sensor_msgs::Imu::ConstPtr &imu_raw)
    {
        ...
        sensor_msgs::Imu thisImu = imuConverter(*imu_raw); // 将imu信息转换到雷达坐标系下表达,其实也就是想去获得雷达运动的加速度、角速度和姿态信息

        // 这里有俩个队列,作用不同,一个用来执行预积分和位姿的优化.一个用来更新最新的imu状态
