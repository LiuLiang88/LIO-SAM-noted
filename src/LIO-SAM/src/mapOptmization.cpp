...
public:
    ...
    mapOptimization()
    {
        ...
        isam = new ISAM2(parameters); // 初始化isam2优化
    ...
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr &msgIn)
    {
        ...
        // 这里做了一个下采样,也就是当前帧距离上一帧的时间大于0.15s时才进行处理,可以根据自己的配置进行调整
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            timeLastProcessing = timeLaserInfoCur;

            // 更新当前数据帧对应的位姿的初始估计 因为一帧点云在地图上的最佳位姿这是一个优化问题,但是初值的选取是非常重要的
            updateInitialGuess();
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void updateInitialGuess()
    {
        ...
        // transformTobeMapped中保存的是上一帧优化后的位姿,是按照x,y,z,yaw等保存的
...
int main(int argc, char **argv)
{
    ...
    std::thread loopthread(&mapOptimization::loopClosureThread, &MO);                // 定义了一个回环线程
    std::thread visualizeMapThread(&mapOptimization::visualizeGlobalMapThread, &MO); // 定义了一个地图可视化线程
